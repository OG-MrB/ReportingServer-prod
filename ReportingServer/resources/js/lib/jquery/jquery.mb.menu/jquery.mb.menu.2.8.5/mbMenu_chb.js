  ...
  $.fn.extend({
    buildMbMenu: function(op,m,type,e){
      ...
      $(this.voices).each(function(i){
        ...
        var line="<table id='"+m+"_"+i+"' class='line"+(isTitle?" title":"")+"' cellspacing='0' cellpadding='0' border='0' style='width:100%;' width='100%'>";
		line += "<tr>"+imgPlace+"<td class='voice' ";
		if(!op.options.wrapContent){
			line += " nowrap ";
		}
		line += "></td></tr>";
		line += "</table>";
	  ...
	  //determine menu and submenu width
	  var updMenuWidth = op.options.menuWidth;
	  switch(type){
        case "sm":
		  if(op.options.subMenuWidth!=0){
			updMenuWidth = op.options.subMenuWidth;		  
		  }
          break;
        case "cm":
		  break;
        default:
		  updMenuWidth = updMenuWidth;
          break;
      }
	  
      $(this.menuContainer).css({
        minWidth:updMenuWidth
      });
	  
      //if ($.browser.msie) $(this.menuContainer).css("width",$(this.menuContainer).width()+2);	  
	  if ($.browser.msie) $(this.menuContainer).css("width",updMenuWidth+2);
...
//alert('2');
	var my_elem_offset = $("#"+op.options.containment).offset();	
	if(my_elem_offset==null){
      var wh= (op.options.containment=="window")?$(window).height():$("#"+op.options.containment).outerHeight();
      var ww=(op.options.containment=="window")?$(window).width():$("#"+op.options.containment).outerWidth();
	}else{
